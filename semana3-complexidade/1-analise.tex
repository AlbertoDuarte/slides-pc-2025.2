\begin{frame}{Análise assintótica}
    \metroset{block=fill}
    ...
    \begin{block}{Definição}
        ...
        \end{align*} \pause
        Se $f(n) \in O(g(n))$, dizemos que $g(n)$ limita superiormente $f(n)$. Escrevemos $f(n) = O(g(n))$. \pause Dizemos que $f(n) = \Theta(g(n))$ se $O(f(n)) = O(g(n))$.
    \end{block} \pause

    \begin{block}{Exemplos}
        \begin{itemize}
            \item[(i)] $f(n) = 2n^2 + 3n + 7 = O(n^2)$. \pause
