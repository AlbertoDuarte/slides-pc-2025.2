\begin{frame}{Teorema Mestre}
    ...
    \begin{Theorem}[Teorema Mestre]
        Sejam $a \geq 1$ e $b \geq 1$ constantes, $f(n)$ uma função, e $T(n)$ definida nos inteiros não-negativos pela recorrência
            \[ T(n) = aT(n/b) + f(n) \ , \]
        interpretando $n/b$ como $\lceil n/b \rceil$ ou $\lfloor n/b \rfloor$. Então $T(n)$ tem os seguintes limites:
            \begin{itemize}
                \item[(i)] Se $f(n) = O(n^{\log_b a - \varepsilon})$ para alguma constante $\varepsilon > 0$, então $T(n) = \Theta(n^{\log_b a})$.
                
                \item[(ii)] Se $f(n) = \Theta(n^{\log_b a})$, então $T(n) = \Theta(n^{\log_b a} \log n)$.
